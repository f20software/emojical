...
class YearBoxView : UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for bit in bits {
            ...
            if index < 6 {
                squareRect = squareRect.offsetBy(dx: 0, dy: step)
                index += 1
            } else {
                squareRect = squareRect.offsetBy(dx: step, dy: -step*6)
                index = 0
            }
        }
    }
