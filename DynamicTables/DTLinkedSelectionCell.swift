...
// A cell with selection list on the right side bound to an object value
// --- [ Text: Value > ]
class DTLinkedSelectionCell: DTSelectionCell {
    
    // are linked cell shown right now?
    var linkedCellsShown: Bool = false
    
    // Set this property to an index of an item which is required to be selected to show linked cells
    var showLinkedSwitchValue: Int = 0
    ...
    override func loadValue() {
        ...
        if (selectedItemIndex == showLinkedSwitchValue)
        {
            row = row + 1
    ...
    override func valueSelected(viewController: UIViewController, value: String) {
        super.valueSelected(viewController: viewController, value: value)
        
        guard linkedCells != nil else { return }
